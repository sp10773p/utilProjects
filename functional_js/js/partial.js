...
!function (G) {
    ...
    if (typeof /./ != 'function' && typeof Int8Array != 'object')
        _.is_fn = _.is_function = _.isFunction = function (obj) {
    ...
    _.memoize2 = function (mid) {
        return function (fn) {
            ...
            return function (obj) {
                return _.has(obj._memoize || (obj._memoize = function () {
                }), memoize_id) ?
    ...
    function reduce(body, _async) {
        return function f(d, i, m) {
            ...
            else var data = arguments[arguments.length - 3], iter = Iter(arguments, true),
    ...
    _.max = collf(function (data, iter, ks, X, i, l, k) {
        ...
        while (++i < l) if (evd < (evd2 = iter(data[k = ks ? ks[i] : i], k, data))) {
            res = data[k];
            evd = evd2
        }
        return res;
    ...
    _.sel = _.select = function f(start, sel) {
        ...
        return sel && _.reduce(sel.split(/\s*->\s*/), function (mem, key) {
            ...
            return !key.match(/^\((.+)\)/) ? (!key.match(/\[(.*)\]/) ? mem[key] : function (mem, numbers) {
                ...
                return !e ? mem[s < 0 ? mem.length + s : s] : slice.call(mem, s < 0 ? mem.length + s : s, e < 0 ? mem.length + e : e + 1);
    ...
    if (typeof define === 'function' && define.amd) define('partial', [], function () {
        return _;
    });
